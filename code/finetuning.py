...
def main(config):
    ...
    validation_dataset_path = "../data/validation_dataset.csv"
    project_name = "odqa_finetuning"
    entity_name = "halfchicken_p2"
    run_name = "klue-roberta-finetuning"
    ...
    if config.wandb.use:
        wandb.init(project=project_name, entity=entity_name, name=run_name + f'{model_args.model_name_or_path}_ep{tr_args.max_epoch}_bs{tr_args.batch_size}_lr{tr_args.learning_rate}')
...
def run_mrc(
    ...
    def prepare_train_features(examples):
        if isinstance(examples[answer_column_name][0], str):
            examples[answer_column_name] = [ast.literal_eval(answer) for answer in examples[answer_column_name]]
    ...
    def post_processing_function(examples, features, predictions, training_args):
        ...
        if training_args.do_predict:
            ...
            references = [{"id": ex["id"], "answers": ast.literal_eval(ex[answer_column_name]) 
                       if isinstance(ex[answer_column_name], str) else ex[answer_column_name]} 
                      for ex in examples]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_path", default="config/finetuning_config.yaml", type=str, help=""
    )
